# ✨ Cloud IAM: Qwik Start || GSP074  
[![Lab Link](https://img.shields.io/badge/Open_Lab-Cloud_Skills_Boost-4285F4?style=for-the-badge&logo=google&logoColor=white)](https://www.cloudskillsboost.google/focuses/1773?parent=catalog)
...
1. **Log in with USERNAME 1**  
   - Use the provided **Username 1** and password to log in to [Google Cloud Console](https://console.cloud.google.com/)  
   - Activate Cloud Shell from the top-right (📟)  

2. **Run the following commands in Cloud Shell**  
   ```bash
   curl -LO raw.githubusercontent.com/TechXNinjas/Cloud-IAM-Qwik-Start/main/TechXNinjas.sh
   sudo chmod +x TechXNinjas.sh
   ./TechXNinjas.sh
   ```

3. **Enter USERNAME 2**  
   - Make sure to **copy and paste the provided USERNAME 2** when prompted in the script.
